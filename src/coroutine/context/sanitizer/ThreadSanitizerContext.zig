...
const tsan = struct {
    extern "c" fn __tsan_create_fiber(flags: c_uint) void;
...
pub fn init(self: *ThreadSanitizerContext, _: Stack) void {
    ...
    const formatted = std.fmt.bufPrint("Fiber from ThreadSanitizerContext @{*}", .{self}) catch unreachable;
    buf[formatted.len] = 0;
    tsan.__tsan_set_fiber_name(self.fiber, &buf);
}
